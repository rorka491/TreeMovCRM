...
function ScheduleByDay() {
    ...
    useEffect(() => {
        const minHour = Math.max(0, lessons.reduce((result, lesson) => Math.min(result, parseInt(lesson.start_time.split(":")[0])), 24) - 1)
        const maxHour = Math.min(24, lessons.reduce((result, lesson) => Math.max(result, parseInt(lesson.end_time.split(":")[0])), 0) + 1)

        setHours(Array.from({ length: maxHour - minHour }, (_, i) => (i + minHour)))
    }, [lessons])
