	...
	return (
		<section>
			<div className="grid grid-cols-[1fr_5fr_3fr_3fr_4fr_5fr_1fr] mb-4 text-gray-600 gap-2">
				{headers.map((title, i) => (
					<div key={i} className={(i === 0 || i === 1 ? "text-sm text-black" : "text-xs font-medium") + " truncate"}>
						{title}
					</div>
				))}
			</div>
			<ul className="max-h-[40vh] overflow-y-auto flex flex-col">
				{employees.map((employee) => (
					<li key={employee.id} className="grid grid-cols-[1fr_5fr_3fr_3fr_4fr_5fr_1fr] items-center text-xs border-y border-[#D9D9D9] hover:bg-gray-100 transition *:py-2.5 *:truncate">
						<div>{employee.id}</div>
