...
const Sidebar = ({ isVisible, setIsVisible, setSection }) => {
	...
	return (
		<div className="relative">
			<aside id="sidebar" className={`w-72 h-screen text-white flex flex-col justify-between transition-transform duration-300 bg-[#7816db] ease-in-out top-0 left-0 ${isVisible ? "" : "hidden"}`}>
				<div>
					...
					<nav className="mt-2 mr-12 ml-6">
						<ul className="space-y-1" style={{ fontFamily: "Arial, Helvetica, sans-serif", fontSize: "14pt", fontWeight: "bold" }}>
							{links.map((link) => (
								<li key={link.path}>
									<Link to={link.path} className={`text-[16px] block px-6 py-3 rounded-lg hover:bg-purple-600 ${"/" + activeSection === link.path ? "bg-[#4b0096]" : ""}`}>
										{link.label}
									</Link>
								</li>
							))}
				...
				<button id="hideBtn" onClick={() => setIsVisible(false)} className="absolute top-1/2 right-0 transform -translate-y-1/2 bg-white bg-opacity-20 text-white py-2 rounded-l-[15px] h-[130px]">
			...
			{!isVisible && (
				<button id="showBtn" onClick={() => setIsVisible(true)} className="fixed top-1/2 left-0 transform -translate-y-1/2 bg-[#7816db] text-white p-2 rounded-r-[15px] h-[130px]">
