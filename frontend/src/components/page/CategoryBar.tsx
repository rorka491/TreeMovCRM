...
function CategoryBar({
    ...
    const searchActive =
        search || searchPlaceholder || onSearch || onSearchInputChange
        ...
        <div className="flex text-[16px] items-center justify-between border-b border-t border-gray-300">
            <ul className="flex space-x-5">
                {categories.map(({ url, label }) => (
                    <li
                        key={url}
                        className={`px-3 py-[10px] cursor-pointer border-b-2 ${activeSection === url || activeSection === label ? 'border-[#7816db]' : 'border-transparent hover:border-[#7816db]'}`}
                    >
