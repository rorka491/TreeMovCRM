...
export function FilterBar({ filterData }) {
	const [selected, setSelected] = useState(Object.fromEntries(filterData.map(filter => [filter.id, filter.default])))
	...
	return (
		<div className="flex grid-flow-col items-end gap-x-2.5 w-full bg-white p-4 rounded-[12.5px] *:rounded-[12.5px]">
			...
			<button className="text-xs px-3 font-medium text-purple-600 duration-200 border border-purple-600 rounded-lg py-2.5 hover:bg-purple-100">Экспорт</button>
		</div>
	)
}