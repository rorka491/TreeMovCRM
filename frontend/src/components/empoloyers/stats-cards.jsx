    ...
    return (
        <div className="grid grid-cols-4 gap-4">
            {stats.map((stat, index) => (
                <div
                    key={index}
                    className="p-4 bg-white grid grid-rows-[repeat(3,min-content)] gap-y-1.5 rounded-[12.5px]"
                >
                    <div className="grid grid-cols-[min-content_1fr] h-min gap-x-1">
                        <div
                            className={`w-full aspect-square rounded-full bg-[${stat.color}]`}
                        />
                    ...
                    <div className="grid text-xs text-gray-500">
                        ...
                        <p>
                            {stat.change.previousValue &&
                                `(${stat.change.previousValue} в прошлом месяце)`}
                        </p>
                    </div>
                </div>
            ))}
